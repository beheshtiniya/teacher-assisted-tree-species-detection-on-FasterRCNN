...
Usage:
    $ python path/to/all.py --assets coco128.yaml --weights yolov5s.pt --img 640
"""
...
class Trainer:
    ...
    def build_model(self, cfg, device):
        ...
        for k, v in self.model.named_parameters():
            v.requires_grad = True  # all all layers
        ...
        # Resume metadata. Optimizer/scheduler/scaler are restored later, after they exist.
        self.start_epoch = 0
        self.resume_best_fitness = 0.0
        self.resume_last_opt_step = -1
        pretrained = cfg.weights.endswith('.pt') and not cfg.reinitial
        if pretrained:
            # PATCH: checkpoint metadata may explicitly contain None
            _best_fitness_value = ckpt.get('best_fitness', 0.0)
            self.resume_best_fitness = (
                float(_best_fitness_value)
                if _best_fitness_value is not None else 0.0
            )
            _last_opt_step_value = ckpt.get('last_opt_step', -1)
            self.resume_last_opt_step = (
                int(_last_opt_step_value)
                if _last_opt_step_value is not None else -1
            )
            ...
            # Epochs
            _checkpoint_epoch = ckpt.get('epoch', -1)
            self.start_epoch = (
                int(_checkpoint_epoch) + 1
                if cfg.resume and _checkpoint_epoch is not None else 0
            )
            if cfg.resume:
            ...
            if self.epochs < self.start_epoch:
                ...
                self.epochs += int(_checkpoint_epoch)  # finetune additional epochs
    ...
    def build_optimizer(self, cfg,optinit=True,weight_masks =None,ckpt=None):
        ...
        if not cfg.Model.RepOpt:
            if cfg.adam:
                ...
                self.optimizer = AdamW(g_bnw, lr=cfg.hyp.lr0,betas=(cfg.hyp.momentum, 0.999))
            else:
                self.optimizer = SGD(g_bnw, lr=cfg.hyp.lr0, momentum=cfg.hyp.momentum, nesterov=True)
            self.optimizer.add_param_group({'params': g_w, 'weight_decay': weight_decay})  # add g1 with weight_decay
            self.optimizer.add_param_group({'params': g_b})  # add g2 (biases)
        else:
        ...
        if ckpt is not None and ckpt.get('optimizer') is not None:
            LOGGER.info("Loading optimizer state from resume checkpoint")
    ...
    def set_env(self, cfg, device, LOCAL_RANK, RANK, WORLD_SIZE, callbacks):
        ...
        # SSL-aware checkpoint paths. last_resume.pt is never optimizer-stripped.
        self.last_resume = w / 'last_resume.pt'
        self.best_student = w / 'best_student.pt'
        self.best_ema = w / 'best_ema.pt'
        ...
        # train_path, val_path = self.data_dict['all'], self.data_dict['val']
        self.data_dict = {}
        self.data_dict['all'] = cfg.Dataset.train
    ...
    def after_epoch(self, callbacks, val):
        if self.RANK in [-1, 0]:
            ...
            self.current_fitness = float(np.asarray(fi).reshape(-1)[0])  # weighted combination of [P, R, mAP@.5, mAP@.5-.95]
    ...
    def train(self, callbacks, val):
        """Train without automatic early stopping.

        Set environment variable EFFICIENTTREE_STOP_EPOCH to stop cleanly after a
        staged epoch while preserving the original scheduler horizon. Example:
        cfg.epochs=70 and EFFICIENTTREE_STOP_EPOCH=30 runs only epochs 0..29,
        saves last_resume.pt, and can later resume toward epoch 50 or 70.
        """
        t0 = time.time()
        self.last_opt_step = int(getattr(self, 'resume_last_opt_step', -1))
        self.results = (0, 0, 0, 0, 0, 0, 0)
        self.best_fitness = float(getattr(self, 'resume_best_fitness', 0.0))

        stop_epoch_text = os.environ.get('EFFICIENTTREE_STOP_EPOCH', '').strip()
        stop_epoch = int(stop_epoch_text) if stop_epoch_text else 0
        if stop_epoch > 0:
            if stop_epoch <= self.start_epoch:
                raise ValueError(
                    f'EFFICIENTTREE_STOP_EPOCH={stop_epoch} must be greater than '
                    f'the resume start epoch {self.start_epoch}.'
                )
            stop_epoch = min(stop_epoch, self.epochs)
            LOGGER.info(
                f'Staged training enabled: stop cleanly after epoch {stop_epoch} '
                f'while scheduler horizon remains {self.epochs} epochs.'
            )
        ...
        for self.epoch in range(self.start_epoch, self.epochs):
            ...
            if stop_epoch > 0 and (self.epoch + 1) >= stop_epoch:
                LOGGER.info(
                    f'Staged stop reached after epoch {self.epoch + 1}. '
                    f'Resume later from {self.last_resume}.'
                )
                break
        ...
        if self.RANK in [-1, 0]:
            LOGGER.info(
                f'\n{self.epoch - self.start_epoch + 1} epochs completed in '
