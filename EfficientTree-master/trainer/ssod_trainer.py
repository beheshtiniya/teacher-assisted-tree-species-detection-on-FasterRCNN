...
class SSODTrainer(Trainer):
    def __init__(self, cfg, device, callbacks, LOCAL_RANK, RANK, WORLD_SIZE):
        ...
        # Only a true resume checkpoint should restore optimizer/scheduler/scaler.
        self.build_optimizer(cfg, ckpt=ckpt if cfg.resume else None)
        self.build_dataloader(cfg, callbacks)
       
        LOGGER.info(f'Image sizes {self.imgsz} all, {self.imgsz} val\n'
    ...
    def build_optimizer(self, cfg, optinit=True, weight_masks=None, ckpt=None):
        ...
        if cfg.SSOD.multi_step_lr:
            ...
            LOGGER.info(f'SSOD MultiStepLR milestones: {milestones}')
            self.scaler = amp.GradScaler(enabled=self.cuda)
            if ckpt is not None and ckpt.get('scheduler') is not None:
                try:
                    self.scheduler.load_state_dict(ckpt['scheduler'])
                except Exception as exc:
                    LOGGER.warning(f'Could not restore SSOD scheduler state: {exc}')
            if ckpt is not None and ckpt.get('scaler') is not None:
                try:
                    self.scaler.load_state_dict(ckpt['scaler'])
                except Exception as exc:
                    LOGGER.warning(f'Could not restore SSOD AMP scaler state: {exc}')

    def build_model(self, cfg, device):
        """Build student, pseudo-label teacher EMA, and reporting semi-EMA.

        New resume checkpoints keep these three networks in separate keys:
        model (student), ema (teacher), and semi_ema (secondary EMA).
        """
        ...
        if pretrained:
            ...
            LOGGER.info(
                f'Transferred {len(csd)}/{len(self.model.state_dict())} items from {weights}'
            )
        else:
        ...
        self.ema = ModelEMA(self.model)  # teacher used to create pseudo-labels
        self.semi_ema = None            # secondary EMA used only for separate reporting
        self.start_epoch = 0
        self.resume_best_fitness = 0.0
        self.resume_last_opt_step = -1
        self.best_student_fitness = 0.0
        self.best_ema_fitness = 0.0

        def load_model_state(target_model, source_model, label):
            if source_model is None:
                return
            try:
                source_state = (
                    source_model.float().state_dict()
                ...
                filtered = {
                    ...
                    if k in target_state and v.size() == target_state[k].size()
                }
                target_state.update(filtered)
                target_model.load_state_dict(target_state, strict=False)
                LOGGER.info(f'Loaded {len(filtered)} tensors into {label}')
            except Exception as exc:
                LOGGER.warning(f'Could not restore {label}: {exc}')

        if pretrained and cfg.resume:
            self.start_epoch = int(ckpt.get('epoch', -1)) + 1
            assert self.start_epoch > 0, (
                f'{weights} does not contain a valid resume epoch.'
            )

            # New checkpoints store the actual pseudo-label teacher in `ema`.
            # `ema_teacher` is accepted for forward compatibility.
            teacher_source = ckpt.get('ema_teacher', ckpt.get('ema'))
            load_model_state(self.ema.ema, teacher_source, 'teacher EMA')
            self.ema.updates = int(ckpt.get('updates', 0))

            if self.start_epoch >= self.cfg.hyp.burn_epochs:
                if self.cosine_ema:
                    self.semi_ema = CosineEMA(
                        self.ema.ema,
                        decay_start=self.cfg.SSOD.ema_rate,
                        total_epoch=max(1, self.epochs - self.cfg.hyp.burn_epochs),
                    )
                else:
                    self.semi_ema = SemiSupModelEMA(
                        self.ema.ema, self.cfg.SSOD.ema_rate
                    )
                semi_source = ckpt.get('semi_ema', ckpt.get('ema'))
                load_model_state(self.semi_ema.ema, semi_source, 'secondary EMA')
                if hasattr(self.semi_ema, 'updates'):
                    self.semi_ema.updates = int(ckpt.get('semi_ema_updates', 0))

            # PATCH: resume metadata may explicitly contain None
            _student_fitness_value = ckpt.get(
                'best_student_fitness', ckpt.get('best_fitness', 0.0)
            )
            _ema_fitness_value = ckpt.get('best_ema_fitness', 0.0)
            self.best_student_fitness = (
                float(_student_fitness_value)
                if _student_fitness_value is not None else 0.0
            )
            self.best_ema_fitness = (
                float(_ema_fitness_value)
                if _ema_fitness_value is not None else 0.0
            )
            self.resume_best_fitness = self.best_student_fitness
            self.resume_last_opt_step = int(ckpt.get('last_opt_step', -1))
            LOGGER.info(
                f'Resuming SSL at epoch {self.start_epoch}; '
                f'best student fitness={self.best_student_fitness:.6f}, '
                f'best EMA fitness={self.best_ema_fitness:.6f}'
            )
        else:
            # Optional pretrained EMA initialization, without treating it as resume.
            if pretrained and ckpt.get('ema') is not None:
                load_model_state(self.ema.ema, ckpt.get('ema'), 'pretrained EMA')
                self.ema.updates = int(ckpt.get('updates', 0))
    ...
    def build_dataloader(self, cfg, callbacks):
        ...
        # Trainloader
        self.train_loader, self.dataset = create_dataloader(self.data_dict['all'], self.imgsz, self.batch_size // self.WORLD_SIZE, gs, self.single_cls,
                                              hyp=cfg.hyp, augment=True, cache=cfg.cache, rect=cfg.rect, rank=self.LOCAL_RANK,
                                              workers=cfg.Dataset.workers, prefix=colorstr('all: '), cfg=cfg)
    ...
    def after_epoch(self, callbacks, val):
        """Validate and checkpoint Student and secondary EMA independently."""
        if (
            self.cfg.SSOD.pseudo_label_type == 'LabelMatch'
        ...
        if not self.noval or final_epoch:
            LOGGER.info(f'\n[Epoch {self.epoch + 1}] Validation: STUDENT')
            student_results, student_maps, _, cls_thr = val.run(
                self.data_dict,
            ...
            LOGGER.info(f'[Epoch {self.epoch + 1}] Validation: {report_ema_name}')
            ema_results, ema_maps, _, _ = val.run(
                self.data_dict,
                batch_size=self.batch_size // self.WORLD_SIZE * 2,
                imgsz=self.imgsz,
                model=report_ema_model,
                conf_thres=self.cfg.val_conf_thres,
                single_cls=self.single_cls,
                dataloader=self.val_loader,
                save_dir=self.save_dir,
            ...
            student_results = getattr(self, 'student_results', self.results)
            student_maps = None
            ema_results = getattr(self, 'ema_results', self.results)
            ema_maps = None

        student_fi = float(
            np.asarray(fitness(np.array(student_results).reshape(1, -1))).reshape(-1)[0]
        )
        ema_fi = float(
            np.asarray(fitness(np.array(ema_results).reshape(1, -1))).reshape(-1)[0]
        )
        student_improved = student_fi > self.best_student_fitness
        ema_improved = ema_fi > self.best_ema_fitness
        if student_improved:
            self.best_student_fitness = student_fi
        if ema_improved:
            self.best_ema_fitness = ema_fi

        # Keep the repository's generic result/logging path tied to the strong Student.
        self.student_results = student_results
        self.ema_results = ema_results
        self.results = student_results
        self.best_fitness = self.best_student_fitness
        self.current_fitness = student_fi

        LOGGER.info(
            f'Checkpoint fitness | STUDENT current/best: '
            f'{student_fi:.6f}/{self.best_student_fitness:.6f} | '
            f'EMA current/best: {ema_fi:.6f}/{self.best_ema_fitness:.6f}'
        )
        log_vals = list(self.meter.get_avg())[:3] + list(student_results) + self.lr
        callbacks.run(
            'on_fit_epoch_end',
            log_vals,
            self.epoch,
        ...
        if (not self.nosave) or final_epoch:
            student_model = deepcopy(de_parallel(self.model)).half()
            teacher_model = deepcopy(self.ema.ema).half()
            report_ema_model_copy = deepcopy(
                self.semi_ema.ema if self.semi_ema is not None else self.ema.ema
            ).half()

            compact_student = {
                'epoch': self.epoch,
                'best_fitness': self.best_student_fitness,
                'best_student_fitness': self.best_student_fitness,
                'best_ema_fitness': self.best_ema_fitness,
                'model': student_model,
                'ema': None,
                'updates': self.ema.updates,
                'optimizer': None,
                'wandb_id': None,
                'checkpoint_role': 'student',
                'validation_results': tuple(float(x) for x in student_results),
            }
            compact_ema = {
                'epoch': self.epoch,
                'best_fitness': self.best_ema_fitness,
                'best_student_fitness': self.best_student_fitness,
                'best_ema_fitness': self.best_ema_fitness,
                'model': report_ema_model_copy,
                'ema': None,
                'updates': self.ema.updates,
                'optimizer': None,
                'wandb_id': None,
                'checkpoint_role': 'semi_ema',
                'validation_results': tuple(float(x) for x in ema_results),
            }
            resume_ckpt = {
                'epoch': self.epoch,
                'best_fitness': self.best_student_fitness,
                ...
                'model': student_model,
                'ema': teacher_model,
                'semi_ema': report_ema_model_copy,
                'updates': self.ema.updates,
                'semi_ema_updates': getattr(self.semi_ema, 'updates', 0),
                ...
                'student_results': tuple(float(x) for x in student_results),
                'ema_results': tuple(float(x) for x in ema_results),
            }

            # Compatibility files: best.pt and last.pt now always contain Student.
            torch.save(compact_student, self.last)
            torch.save(resume_ckpt, self.last_resume)
            if student_improved:
                torch.save(compact_student, self.best_student)
                torch.save(compact_student, self.best)
                LOGGER.info(f'Saved new best STUDENT checkpoint: {self.best_student}')
            if ema_improved:
                torch.save(compact_ema, self.best_ema)
                LOGGER.info(f'Saved new best EMA checkpoint: {self.best_ema}')
            if (
                self.epoch > 0
                and self.save_period > 0
                and self.epoch % self.save_period == 0
            ):
                periodic = self.save_dir / 'weights' / f'epoch{self.epoch}_resume.pt'
                torch.save(resume_ckpt, periodic)

            del compact_student, compact_ema, resume_ckpt
            del student_model, teacher_model, report_ema_model_copy
            callbacks.run(
                'on_model_save',
                self.last,
                self.epoch,
    ...
    def train_without_unlabeled(self, callbacks):
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ni = i + self.nb * self.epoch  # number integrated batches (since all start)
    ...
    def after_train(self, callbacks, val):
        """Finalize compact inference checkpoints; preserve last_resume.pt intact."""
        student_results = (0, 0, 0, 0, 0, 0, 0)
        if self.RANK in [-1, 0]:
            compact_files = [self.last, self.best, self.best_student, self.best_ema]
            for checkpoint_path in compact_files:
                if checkpoint_path.exists():
                    strip_optimizer(checkpoint_path)

            evaluation_targets = [
                ('BEST STUDENT', self.best_student),
                ('BEST EMA', self.best_ema),
            ]
            for label, checkpoint_path in evaluation_targets:
                if not checkpoint_path.exists():
                    continue
                LOGGER.info(f'\nFinal validation of {label}: {checkpoint_path}')
                results, _, _, _ = val.run(
                    self.data_dict,
                    batch_size=self.batch_size // self.WORLD_SIZE * 2,
                    imgsz=self.imgsz,
                    model=attempt_load(checkpoint_path, self.device).half(),
                    conf_thres=self.cfg.val_conf_thres,
                    iou_thres=0.65,
                    single_cls=self.single_cls,
                    dataloader=self.val_loader,
                ...
                if label == 'BEST STUDENT':
                    student_results = results

            callbacks.run('on_train_end', self.last, self.best, self.plots, self.epoch)
            LOGGER.info(f'Results saved to {colorstr("bold", self.save_dir)}')
            if self.last_resume.exists():
                LOGGER.info(f'Resume checkpoint preserved with optimizer: {self.last_resume}')
